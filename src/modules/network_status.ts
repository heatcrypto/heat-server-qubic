...
export async function getLatestTickResponse(context: CallContext): Promise<LatestTickResponse> {
...
export async function getTickDataResponse(context: CallContext, tickNumber: number): Promise<TickDataResponse> {
  const { req, protocol, host, logger } = context
